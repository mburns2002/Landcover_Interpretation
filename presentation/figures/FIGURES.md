...
Deck (Spectral) version of manuscript Figure 3.5, regenerated by pres_20_training_conflict_overlay.py.
The largest grass/shrub-versus-wetland contested zones where both interpreters placed training points but
assigned them to different classes (top three contested patches); the disagreement is conceptual (class
identity), not boundary placement:
