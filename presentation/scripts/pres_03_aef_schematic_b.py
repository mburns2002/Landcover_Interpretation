...
SENSORS = [  # (title, sub, fill, edge, y_center)
    ("Sentinel-2", "optical:  B2, B3, B4, B8, B11", "#d6e6f4", "#0072B2", 5.6),
    ("Landsat 8/9", "optical + thermal:  B2–B6, B8, B10", "#f7ddcf", "#D55E00", 4.15),
...
def _box(ax, x, y, w, h, fill, edge, title, sub, lw=2.2):
    ...
    ax.text(cx, y + h / 2 - 0.22, sub, ha="center", va="center", fontsize=FS_SUB, color="0.3",
            zorder=4)
...
def _stack(ax, cx, cy, n=4, s=0.74, off=0.15):
    bbox = s + (n - 1) * off
    bl = (cx - bbox / 2, cy - bbox / 2)
    for i in range(n - 1, -1, -1):
        ax.add_patch(Rectangle((bl[0] + i * off, bl[1] + i * off), s, s, facecolor=EMB_FILL,
                               edgecolor=EMB_EDGE, linewidth=1.8, zorder=3 + (n - 1 - i)))
...
def main():
    ...
    _arrow(ax, (mx + mw, my + mh / 2), (EMB_CX - 0.72, EMB_CY))
    _stack(ax, EMB_CX, EMB_CY)
    ax.text(EMB_CX, EMB_CY - 1.05, "64-D embedding", ha="center", va="top", fontsize=FS_EMB,
            fontweight="bold", color=TEXT)
    ax.text(EMB_CX, EMB_CY - 1.33, "per 10 m pixel", ha="center", va="top", fontsize=FS_EMB,
            color=MUTED)
    ...
    ax.add_patch(FancyBboxPatch((tx, ty), tw, th, boxstyle="round,pad=0.02,rounding_size=0.08",
                                ...
                                zorder=1))
    ax.text(tx + tw / 2, ty + th - 0.24, "Training targets", ha="center", va="center",
            fontsize=FS_SECTION - 1, fontweight="bold", color=TEXT, zorder=4)
    ax.text(tx + tw / 2, ty + th - 0.5, "(training only)", ha="center", va="center", fontsize=FS_SUB,
