#!/usr/bin/env python3
"""pres_15_embedding_ops_version_4: symbolic equations with abstract embedding textures.
...
Embeddings are abstract generated textures (inspired by the uploaded AlphaEarth maps, not the real
data). The delta output keeps 64 dimensions (drawn as a stack, RdBu); the dot product collapses to a
single value (one grayscale square).
...
Output (PNG only): presentation/figures/pres_15_embedding_ops_version_4.png
"""
...
def main():
    ...
    # header + year labels over the first (delta) row
    ...
    fig.savefig(os.path.join(OUT, "pres_15_embedding_ops_version_4.png"), dpi=300)
    plt.close(fig)
    print("wrote pres_15_embedding_ops_version_4.png")
