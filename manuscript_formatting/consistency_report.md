...
- Figure 2.9 (classified-map speckle) was regenerated from the current 180-cell pipeline, so it no
  longer draws from the superseded arm. Table S3 (map speckle) was also regenerated on the current
  180-cell basis (reports/model_comparison_current/) and now matches Figure 2.9.
